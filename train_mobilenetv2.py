...
def caculte_val_loss(model,data_loader,device):
    val_losses=[]
    for images,targets in data_loader:
        ...
        with torch.no_grad():
            ...
            print(loss_dict[0].values())
            loss_value=sum(loss_dict[0])
        val_losses.append(loss_value.item())
    val_mean_loss=torch.tensor(val_losses).mean()
...
def main():
    ...
    for epoch in range(init_epochs):
        ...
        #val_loss=caculte_val_loss(model,val_data_loader,device)
        train_loss.append(mean_loss.item())
        learning_rate.append(lr)
        writer.add_scalar("train_loss",mean_loss.item(),epoch)
        ...
        writer.add_scalar("val_mAp",coco_info[1],epoch)
